...
async def create_ticket():
    async with httpx.AsyncClient() as client:
        res = await client.post(f"{BASE_URL}/tickets", json={
            "name": "Test User",
...
async def validate_ticket(token, ticket_id):
    async with httpx.AsyncClient() as client:
        res = await client.post(f"{BASE_URL}/validate_ticket",
...
async def export_tickets(token):
    async with httpx.AsyncClient() as client:
        res = await client.get(f"{BASE_URL}/admin/export",
            headers={"Authorization": f"Bearer {token}"}
        )
        res.raise_for_status()
        return res.json()
...
async def run_tests():
    ...
    assert any(t["ticket_id"] == ticket_id for t in tickets), "❌ Ticket not found in export"
